...
- **Input** — modern by default: the [kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) is negotiated so modifiers arrive on *every* key (including Shift+letter), bare modifiers and key press/repeat/release are reported, with automatic fallback to broad legacy decoding (xterm normal/application, SS3, rxvt, Linux console, vt220 tilde forms + CSI modifier suffixes) on terminals that don't support it. SGR (1006) mouse and resize events too. `LT_INPUT_COMPAT` restores termbox2 control-byte semantics for drop-in code.
...
| `kbd` | on-screen keyboard + live event inspector (keys, modifiers, `action`, bare modifiers) |
